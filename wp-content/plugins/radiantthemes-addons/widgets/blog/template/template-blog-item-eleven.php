...
// POST FORMAT QUOTE.
$output                 .= '<!-- blog-item -->';
$output                 .= '<article id="blog-item-'. get_the_ID() .'" class="blog-item wow slideInUp">';
	$output             .= '<div class="holder matchHeight">';
		...
		$output         .= '<div class="data">';
			...
			$output     .= '<ul class="meta">';
				$output .= '<li class="date" >' . get_the_date() . esc_html__( ' by ', 'radiantthemes-addon' ) . get_the_author() . '</li>';
			$output     .= '</ul>';
		$output         .= '</div>';
	$output             .= '</div>';
$output                 .= '</article>';
